...
const lines = [
  { label: "ESPA competitiveness", value: "Fit", ready: true },
  { label: "EIC Accelerator", value: "Fit", ready: true },
  { label: "Horizon Europe", value: "Review", ready: false },
];

export function FundingHeroVisual() {
  ...
  return (
    <SceneFrame className="h-full bg-background/80 p-1.5 shadow-[var(--shadow-lg)]">
      <SceneHeader subtitle="Open calls" title="Matched today">
        <UiBadge variant={ready ? "secondary" : "outline"}>{ready ? "2 fits" : "Checking"}</UiBadge>
      </SceneHeader>
      ...
      <div className="border-t border-border px-3 py-2">
        <p className="text-[11px] text-muted-foreground">{ready ? "Alerts on for this company" : "Matching the company file"}</p>
      </div>
    </SceneFrame>
  );
}
