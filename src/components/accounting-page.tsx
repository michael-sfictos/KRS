...
import { ArrowRight, Clock3, MessageSquareText, PanelsTopLeft } from "lucide-react";

import { AccountingFeatureShowcase } from "@/components/accounting-features";
import { AccountingHeroVisual } from "@/components/accounting-hero-visual";
import { AccountingProcessSection } from "@/components/accounting-process-section";
...
const heroBullets = [
  { icon: Clock3, text: "Time back each month, because the admin is handled" },
  { icon: MessageSquareText, text: "Direct access to your accountant, with a 24-hour response" },
  { icon: PanelsTopLeft, text: "Books, VAT, myDATA and reporting in one operating file" },
];
...
export function AccountingPage() {
  return (
    <main className="min-h-screen overflow-x-clip bg-background text-foreground">
      ...
      <Hero />
      <Pillars />
      <AccountingFeatureShowcase />
      <StatsSection />
      <AccountingProcessSection />
      <AccountingContactCta />
      <Benefits page={accountingPage} />
      <SwitchProcess page={accountingPage} />
      <TestimonialsCarousel />
      <ServiceFaq page={accountingPage} />
      <SlimCta ctaLabel={ctaLabel} heading="Ready for books that stay current?" />
      ...
      <div className="mx-auto grid max-w-[1400px] items-center gap-10 py-12 lg:grid-cols-[0.92fr_1.08fr] lg:gap-16 lg:py-16">
        <div className="flex flex-col justify-center">
          ...
          <h1 className="mt-5 max-w-[18ch] text-balance text-4xl font-normal leading-[1.08] tracking-tight sm:text-5xl lg:text-6xl">
            <Reveal play="mount">{accountingPage.title}</Reveal>
          </h1>
          <RevealFade
